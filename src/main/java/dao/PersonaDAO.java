...
public class PersonaDAO implements PersonaInterface{
	...
	public int registrarPersona(Persona obj) {
		...
		try {
			...
			if(resultado != 2) {
				String sql="insert into Persona(VC_NOMBRE,VC_DNI,VC_ESTADO,DT_FEC_REG,VC_TIPO) values (?,?,'ACT',now(),'CLI')";
	...
	public int actualizarPersona(Persona obj) {
		...
		try {
			...
			String sql="update Persona set VC_NOMBRE = ? , VC_DNI = ?,DT_FEC_MOD=now() where IN_ID_PERSONA = ?";
			pstm=cn.prepareStatement(sql);
			
            pstm.setString(1, obj.getVC_NOMBRE());
            pstm.setString(2, obj.getVC_DNI());
            pstm.setInt(3, obj.getIN_ID_PERSONA());
            resultado=pstm.executeUpdate();
